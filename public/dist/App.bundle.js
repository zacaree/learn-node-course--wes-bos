...
// This library is designed to protect us from XSS attacks when setting innerHTML
...
function typeAhead(search) {
  ...
  searchInput.on('keyup', function (e) {
    ...
    if (e.keyCode === 40 && current) {
      // If down is pressed and there is already an active item in the list then make the next item active or the first item.
      next = current.nextElementSibling || items[0];
    } else if (e.keyCode === 40) {
      // If none are active yet then "next" becomes the first item in the list, making it active.
      next = items[0];
      ...
      // If enter is pressed and the current element has a link
